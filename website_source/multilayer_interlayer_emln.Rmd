...
### R Code
Because this is a directed bipartite network (interlayer links move from layer to layer in time), we will also consider the intralayer links as directed. To do that, we will copnvert each bipartite matrix to a symmetric unipartite matrix (see Figure XXX our paper...). This does not change the network itself because each link is exactly the same, in both directions.

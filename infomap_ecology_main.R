...
pdf('/Users/shai/Dropbox (BGU)/Apps/Overleaf/A dynamical perspective on community detection in ecological networks/figures/comparison_eta_metadata.pdf',12,8)
...
infomap_object$module_affiliation %>%
  group_by(module_level1) %>% summarise(n=n_distinct(node_id))
...
svg('/Users/shai/Dropbox (BGU)/Apps/Overleaf/A dynamical perspective on community detection in ecological networks/figures/modules_directed_model.svg')
...
svg('/Users/shai/Dropbox (BGU)/Apps/Overleaf/A dynamical perspective on community detection in ecological networks/figures/modules_rawdir_model.svg')
plot(network_object$igraph_object, vertex.label=NA, edge.color='black',
     edge.width=log(E(g)$weight+1),
...
NEE2017_modules <- run_infomap_multilayer(NEE2017, relax = F, flow_model = 'directed', silent = T, trials = 100, two_level = T, seed = 497294, temporal_network = T)

#Module persistance
modules_persistence <- new_interlayer_modules %>%
  group_by(module) %>%
...
module_switch <- new_interlayer_modules%>%
...
fig2 <- ggplot(new_interlayer_modules,
...
# Temporal multilayer network: Global relax rates: Carmel's version ---------


relaxrate_sensitivity <- function(file_name, jump, nodes_file){
  "A function that runs Infomap and returns the network's modular structure at increasing relax-rates.
  file_name: A txt file of infomap's input with intra layer edges only, with the following columns: layer, node_from, node_to
  jump: interval for calculating the relax rate
  nodes_file: file with node attributes"

  nodes_data <- read_csv(nodes_file)
  output <- NULL
  for (i in seq(0,1, by = jump)){
    print(i)
    system(paste("./Infomap ", file_name, ".txt . -2 --seed 497294 -N 50 -i multilayer --multilayer-relax-rate ", i, " --multilayer-relax-limit-up 1 --multilayer-relax-limit-down 0 --silent", sep = ""))
    infomap_file <- suppressMessages(read_delim(paste(file_name, "_states.tree", sep=""), delim = ' ', skip = 2, col_names = c('path', 'flow', 'name', 'stateId', 'node_id', 'layer')))
    infomap_file %<>%
      filter(flow>0) %>%
      ...
      mutate("relax_rate"=i) %>%
      left_join(nodes_data, "node_id" )
    organized_infomap_data <- infomap_file %>%
      distinct(module,layer) %>%
      arrange(module,layer)
    x <- c(1,table(organized_infomap_data$module))
    module_birth_layers <- organized_infomap_data %>% slice(cumsum(x)) %>% arrange(layer,module)
    module_renaming <- data.frame(module=module_birth_layers$module, module_renamed = 1:max(module_birth_layers$module))
    organized_infomap_data %<>%
      left_join(module_renaming, 'module') %>%
      full_join(infomap_file, c("module", "layer")) %>%
      select(-module) %>%
      rename(module=module_renamed)
    output <- rbind(output, organized_infomap_data)
  }
  return(output)
}


#Analyzing the network structure using different relax rates
relaxrate_modules <- relaxrate_sensitivity("NEE2017_multilayer", 0.1, "NEE2017_nodes.csv")
...
#2. Modules presistence
presistence <- relaxrate_modules %>%
  group_by(relax_rate, module, layer) %>%
...
tur2016 %<>% filter(altitude==1800) %>% select(from=donor,to=receptor,weight=no.grains)
network_object_observed <- create_network_object(tur2016, directed = T, bipartite = F)
infomap_input_observed <- create_infomap_linklist(network_object_observed)
results_observed <- run_infomap_link_list(infomap_input_observed, infomap_executable='Infomap',
                                          input_file = NULL,
                                          flow_model = 'directed',
                                          silent=T,trials=100, two_level=T, seed=200952, '-k --ftree --markov-time 50')
...
for (i in 501:1000){
  ...
  # Run Infomap
  network_object_simulated <- create_network_object(shuffled, directed = T, bipartite = F)
  infomap_input_simulated <- create_infomap_linklist(network_object_simulated)
  results_simulated <- run_infomap_link_list(infomap_input_simulated, infomap_executable='Infomap',
                                             input_file = NULL,
                                             flow_model = 'directed',
                                             silent=T,trials=100, two_level=T, seed=200952, '-k --ftree --markov-time 50')
...
sum(results_shuffled$L<results_observed$L)/1000 # P value for L
sum(results_shuffled$M>M_obs)/1000 # P value for M
